...
/// # Errors
///
/// Returns a string error if serialization or file writing fails.
...
async fn apply_loaded_session(
    ...
    // Restore transcript.
    {
        ...
        transcript.summary.clone_from(&session.summary);
...
/// Load `autosave.turn` back into the session as an anonymous recovery draft.
///
/// Deletes the autosave file after restoring. The restored session has no
...
pub async fn save_session_as(
    ...
    let filename = suggested_name
        .filter(|s| !s.is_empty())
        .map_or_else(|| "session.turn".to_string(), |s| format!("{s}.turn"));
...
async fn do_save(
    ...
    if final_meta.created_at.is_empty() {
        let saved_at = final_meta.saved_at.clone();
        final_meta.created_at = saved_at;
    }
