...
1. **No mock, ever.** Remove the `mock-llm` Cargo feature and the entire `MockBackend`. The assistant is either connected to a real backend or explicitly _disconnected_; it never echoes and never fabricates a turn.
...
**Cons:** Invisible to Finder/Dock launches — this _is_ the bug. No recovery path inside the app.
