...
function assistantInput(page: Page): ReturnType<Page['locator']> {
  return page.locator('.assistant-input')
}

function assistantHistory(page: Page): ReturnType<Page['locator']> {
  return page.locator('.assistant-history')
}
...
test.describe('AssistantPanel layout', () => {
  test('assistant panel is visible in the app layout', async ({ page, mountApp }) => {
    await mountApp()
    await expect(page.locator('.assistant-panel')).toBeVisible()
  })

  test('assistant history container is present', async ({ page, mountApp }) => {
    await mountApp()
    await expect(assistantHistory(page)).toBeVisible()
  })

  test('assistant input is present', async ({ page, mountApp }) => {
    await mountApp()
  ...
  test('resize handle is present', async ({ page, mountApp }) => {
    await mountApp()
    await expect(page.locator('.assistant-resize-handle')).toBeVisible()
  })
})
...
test.describe('Assistant input behaviour', () => {
  test('typing in the input changes its value', async ({ page, mountApp }) => {
    ...
    await expect(assistantInput(page)).toHaveText('hello Lean')
  })
  ...
  test('Enter sends the message and clears the input', async ({ page, mountApp }) => {
    ...
    await expect(assistantInput(page)).toBeEmpty()
  })
  ...
  test('Shift+Enter inserts a newline instead of sending', async ({ page, mountApp }) => {
    ...
    const value = await assistantInput(page).textContent()
  ...
  test('Option+Enter (Alt+Enter) does not send the message', async ({ page, mountApp }) => {
    ...
    await expect(assistantInput(page)).toHaveText('line one')
  })
})
...
test.describe('Message rendering', () => {
  ...
  test('sent user message appears in message history', async ({ page, mountApp }) => {
    ...
    await expect(assistantHistory(page)).toContainText('hello there')
  })

  test('user message bubble has role indicator', async ({ page, mountApp }) => {
    await sendAndWait(page, { mountApp }, 'ping')
    await expect(page.locator('.assistant-message-user')).toBeVisible()
  })

  test('assistant reply appears after assistant-complete event', async ({ page, mountApp }) => {
    await mountApp()
    await assistantInput(page).click()
    await page.keyboard.type('hello')
    await page.keyboard.press('Enter')

    // The fixture mock fires assistant-complete automatically (see fixtures.ts)
    await expect(page.locator('.assistant-message-assistant')).toBeVisible({ timeout: 3000 })
    await expect(assistantHistory(page)).toContainText('[echo] hello')
  })
  ...
  test('lean code in backticks renders with syntax highlight class', async ({
    ...
    await emitEvent('assistant-complete', {
      role: 'assistant',
    ...
    await expect(page.locator('.assistant-lean-code')).toBeVisible()
  })
...
test.describe('Auto-scroll', () => {
  test('assistant history scrolls to bottom when new message is added', async ({
    page,
    mountApp,
    emitEvent,
  }) => {
    ...
    await expect(page.locator('.assistant-scroll-anchor')).toBeAttached({ timeout: 3000 })
    // The assistant history should be scrolled to the bottom: scrollTop + clientHeight ≈ scrollHeight
    const isAtBottom = await page.locator('.assistant-history').evaluate((el) => {
...
test.describe('Streaming and thinking indicator', () => {
  test('thinking indicator shows while waiting for assistant reply', async ({ page, mountApp }) => {
    // Mount with a mock that does NOT auto-fire assistant-complete
    ...
    await expect(page.locator('.assistant-thinking-indicator')).toBeVisible({ timeout: 2000 })
  })

  test('thinking indicator disappears after assistant-complete', async ({
    page,
    mountApp,
    emitEvent,
  }) => {
    ...
    await emitEvent('assistant-stream-done', null)
    await emitEvent('assistant-complete', {
      role: 'assistant',
      content: '[echo] hello',
      timestamp: Date.now(),
    ...
    await expect(page.locator('.assistant-thinking-indicator')).not.toBeVisible({ timeout: 2000 })
  })
  ...
  test('streaming text appears incrementally in a bubble', async ({
    ...
    await emitEvent('assistant-delta', 'Hello ')
    await emitEvent('assistant-delta', 'world')

    // The streaming bubble should contain the accumulated text
    const streamingBubble = page.locator('.assistant-message-streaming')
    ...
    await expect(page.locator('.assistant-message-assistant')).toContainText('Hello world')
  })
...
test.describe('Resize handle', () => {
  test('drag handle changes input area height', async ({ page, mountApp }) => {
    ...
    const inputArea = page.locator('.assistant-input-area')
